...
(function() {
    Array.prototype.some || (Array.prototype.some = function(g, i) {
        ...
        if ("function" != typeof g)
            throw new TypeError;
        for (var m = 0; m < k; m++)
            if (m in this && g.call(i, this[m], m, this))
                return !0;
        return !1
    });
    ...
    if (!Object.keys) {
        var i = Object, q = Object.prototype.hasOwnProperty, g=!{
            toString: null
        }.propertyIsEnumerable("toString"), p = "toString toLocaleString valueOf hasOwnProperty isPrototypeOf propertyIsEnumerable constructor".split(" "),
        z = p.length;
        i.keys = function(i) {
...
sigma.classes.Cascade = function() {
    ...
    this.config = function(i, q) {
        if ("string" == typeof i && void 0 == q)
            return this.p[i];
        var g = "object" == typeof i && void 0 == q ? i : {};
        "string" == typeof i && (g[i] = q);
        for (var p in g)
            void 0 != this.p[p] && (this.p[p] = g[p]);
        return this
    }
};
...
(function() {
    function i() {
        ...
        this.rescale = function(a, b, d, f) {
            ...
            Math.max(a / Math.max(l - g, 1), b / Math.max(t - s, 1)): Math.min(a / Math.max(l - g, 1), b / Math.max(t - s, 1)), i = (c.p.maxNodeSize || h) / A;
            l += i;
            g -= i;
            t += i;
            s -= i;
            var A = "outside" == c.p.scalingMode ? Math.max(a / Math.max(l - g, 1), b / Math.max(t - s, 1)): Math.min(a / Math.max(l - g, 1), b / Math.max(t - s, 1)), u, k;
        ...
        this.checkHover = function(a, b) {
            ...
            c.nodes.forEach(function(c) {
                if (c.hidden)
                    ...
                    j = c.displaySize;
                    var s = c.hover, t = d < j && f < j && Math.sqrt(d * d + f * f) < j;
    ...
    function p(b) {
        ...
        function c() {
            ...
            u = Math.min(u, 1);
            var b = sigma.easing.quadratic.easeout(u), c = a.ratio;
            a.ratio = c * (1 - b) + h * b;
        ...
        var i = 0, d = 0, w = 0, o = 0, h = 1, n = 0, l = 0, s = 0, t = 0, A = 0, k = 0, u = 0, m=!1;
        ...
        b.addEventListener("mousemove", function(b) {
            ...
            if (a.isMouseDown) {
                var c = a.mouseX - i + w, h = a.mouseY - d + o;
                if (c != a.stageX || h != a.stageY)
                    t = s, k = A, s = c, A = h, a.stageX = c, a.stageY = h, a.dispatch("drag")
            }
            a.dispatch("move");
    ...
    function z(b, j, f, c, a, g, d) {
        function i(a) {
            var b = c, d = "fixed" == h.p.labelSize ? h.p.defaultLabelSize: h.p.labelSizeRatio * a.displaySize;
        ...
        this.task_drawEdge = function() {
            ...
            b;)
                ...
                d = c.source.displayX;
                var g = c.source.displayY, o = c.target.displayX, i = c.target.displayY, l = c.color;
                if (!l)
        ...
        this.task_drawNode = function() {
            for (var c = a.nodes.length, d = 0; d++<h.p.nodesSpeed && h.currentNodeIndex < c;)
                if (h.isOnScreen(a.nodes[h.currentNodeIndex])) {
                    var f = a.nodes[h.currentNodeIndex++], j = Math.round(10 * f.displaySize) / 10, g = b;
                    g.fillStyle = f.color;
                    g.beginPath();
    ...
    function F(b, g) {
        ...
        for (var w = 0; w < m.plugins.length; w++)
            m.plugins[w](this);
        this.draw = function(a, b, c, g) {
            ...
            for (var j in d.domElements)
                "canvas" == d.domElements[j].nodeName.toLowerCase() && (void 0 == g[j] || 0 <= g[j]) && d.domElements[j].getContext("2d").clearRect(0, 0, d.domElements[j].width, d.domElements[j].height);
            ...
            if (a)
                if (1 < a)
                    ...
                    sigma.chronos.addTask(d.plotter.task_drawNode, "node_" + d.id, !1), g=!0, j = "node_" +
            d.id;
            if (c)
                if (1 < c)
                    for (; d.plotter.task_drawLabel(););
                else 
                    j ? sigma.chronos.queueTask(d.plotter.task_drawLabel, "label_" + d.id, j) : sigma.chronos.addTask(d.plotter.task_drawLabel, "label_" + d.id, !1), g=!0, j = "label_" + d.id;
            if (b)
                if (1 < b)
                    for (; d.plotter.task_drawEdge(););
                else 
                    j ? sigma.chronos.queueTask(d.plotter.task_drawEdge, "edge_" + d.id, j) : sigma.chronos.addTask(d.plotter.task_drawEdge, "edge_" + d.id, !1), g=!0, j = "edge_" + d.id;
            d.dispatch("draw");
            d.refresh();
        ...
        b) {
            ...
            if (c != d.width || f != d.height) {
                for (var j in d.domElements)
                    d.domElements[j].setAttribute("width", d.width + "px"), d.domElements[j].setAttribute("height", d.height + "px");
    ...
    sigma.chronos = new function() {
        ...
        sigma.classes.EventDispatcher.call(this);
        var h = this, n=!1, l = 80, k = 0, m = 0, p = 1E3 / l, q = p, u = 0, x = 0, B = 0, z = 0, r = {}, v = [], y = [], C = 0;
    ...
    sigma.tools.drawRoundRect = function(b, g, f, c, a, i, d) {
        ...
        0 <= k.indexOf("top") || 0 <= k.indexOf("right")), h = i && (0 <= k.indexOf("bottomleft") || 0 <= k.indexOf("bottom") || 0 <= k.indexOf("left")), k = i && (0 <= k.indexOf("bottomright") || 0 <= k.indexOf("bottom") || 0 <= k.indexOf("right"));
        b.moveTo(g, f + i);
