...
jQuery.getJSON(GetQueryStringParams("config","../static/graphs/config.json"), function(data, textStatus, jqXHR) {
	config=data;
...
function initSigma(config) {
    ...
    if (data.indexOf("gexf")>0 || data.indexOf("xml")>0)
        a.parseGexf("../static/graphs/"+data,dataReady);
    else
	    a.parseJson("../static/graphs/"+data,dataReady);
    gexf = sigmaInst = null;
}
